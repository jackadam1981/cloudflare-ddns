...
1. **配置文件**：确保在脚本目录下存在 `cfconf.json` 配置文件。可以通过命令行参数 `-c` 指定其他配置文件路径。例如：

   ```bash
   ./ddns.sh -c /path/to/your/config.json
   ```

   如果配置文件不存在，脚本会自动生成一个示例配置文件。
...
- 本脚本使用 `sh` 编写，旨在兼容 OpenWrt 和其他嵌入式 Linux 系统。

## TODO

- 通过使用 `grep`、`sed` 和 `awk` 等工具解析 JSON 数据替换 `jq`，减少对外部依赖的需求。
